...
const memory = { bookings: [], invoices: [], nextInvoiceNumber: 1001, settings: {} };
  ...
  const { rows } = await query(
    `INSERT INTO bookings
       (id, name, phone, email, vehicle, vehicle_desc, package_id, addons, address,
        preferred_date, preferred_time, notes, status, scheduled_at, duration_min, quote,
        crm_notes, source)
    ...
    [
      ...
      record.durationMin, record.quote, record.crmNotes, record.source
    ]
  );
...
async function getSetting(key) {
  if (!hasDatabase) return memory.settings[key] || null;
